 ...
 * <u>Modifications:</u>
 * <ul>
 * <li>03/08/2023 - removed string concatenation in {@link #endSession} and {@link #getHttpRequestParameters}. Also modified {@link #check} to only call {@code toLowerCase} a single time by making it a method local field.</li>
 * </ul>
 *
 * @author Joseph Burris JCCC
 * @author Leroy Norman JCCC
 */
public class SecurityFilter implements Filter {
    ...
    private void endSession(ServletResponse resp, HttpServletRequest request) throws IOException {
        ...
        if(session.getAttributeNames() != null){
            ...
            while(enumer.hasMoreElements()){
                ...
                buffer.append(key).append('=').append(check(key, session.getAttribute(key))).append(' ');
    ...
    private String getHttpRequestParameters(HttpServletRequest request) {
        ...
        if(request.getParameterNames() != null){
            ...
            while(enumer.hasMoreElements()){
                key = enumer.nextElement();
                buffer.append(key).append('=').append(check(key, request.getParameter(key))).append(' ');
            }// end while
    ...
    private String check(String key, Object value) {
        ...
        String result = (lower.contains("ssn") || lower.contains("password")) ? "not_printable" : String.valueOf(value).trim();
