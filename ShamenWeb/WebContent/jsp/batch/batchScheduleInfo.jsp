...
<bean:define id="repeatCodes" name="batchForm" property="repeatCodes"></bean:define>
<bean:define id="selectType" name="batchForm" property="selectType"></bean:define>
<bean:define id="weekDays" name="batchForm" property="weekDays"></bean:define>
<div class="row">
	<div class="col">
		<div class="card bg-dark mb-4 p-3 pb-3">
			<div class="card-header m-1 p-2 border-bottom">
				<h5 class="bs-component m-0 text-white" id="scheduleInfotitle" title="<bean:message key="prompt.batch.schedule.title"/>"><bean:message key="prompt.batch.schedule.title"/></h5>
					<logic:equal value="true" name="batchForm" property="scheduleAddFlag">
				<h6 class="m-0 text-warning" id="scheduleAddNotice"><bean:message key="common.message.schedule.change"/></h6>
				</logic:equal>
			</div>
			<div class="col-md-11 align-self-center">
				<display:table name="sessionScope.batchForm.batchApp.scheduleList" uid="schedules" class="table table-sm table-striped table-dark table-bordered" export="false" pagesize="" defaultsort="4">
								...
								<ul class="dropdown-menu dropdown-menu-small" role="menu">
									<logic:equal value="0" name="schedules" property="scheduleRefId">
										<li  class="dropdown-item">
											<a href="<bean:write name="requestVar" property="contextPath"/>/batchDetailAction.do?method=editSchedule&scheduleListNumber=<bean:write name="schedules" property="listNumber"/>">View / Edit</a>
										</li>
										<li  class="dropdown-item">
											<a href="<bean:write name="requestVar" property="contextPath"/>/batchDetailAction.do?method=deleteSchedule&scheduleListNumber=<bean:write name="schedules" property="listNumber"/>" onclick="return deleteConfirmation()">Delete</a>
										</li>
									</logic:equal>
									<logic:notEqual value="0" name="schedules" property="scheduleRefId">
										<li  class="dropdown-item">
											<a href="<bean:write name="requestVar" property="contextPath"/>/batchDetailAction.do?method=editSchedule&scheduleRefId=<bean:write name="schedules" property="scheduleRefId"/>">View / Edit</a>
										</li>
										<li  class="dropdown-item">
											<a href="<bean:write name="requestVar" property="contextPath"/>/batchDetailAction.do?method=deleteSchedule&scheduleRefId=<bean:write name="schedules" property="scheduleRefId"/>" onclick="return deleteConfirmation()">Delete</a>
										</li>
										<logic:notEqual value="EDT" name="schedules" property="lastRunStatus.resultCd">
											<logic:equal name="schedules" property="activeInd" value="Y">
												<li class="dropdown-item">
													<a href="<bean:write name="requestVar" property="contextPath"/>/batchDetailAction.do?method=updateBatchAppActivate&scheduleRefId=<bean:write name="schedules" property="scheduleRefId"/>&activate=N" onclick="return deactivateConfirmation()" title="Deactivate Batch App Schedule">Deactivate Schedule</a>
												</li>
											</logic:equal>
											<logic:notEqual name="schedules" property="activeInd" value="Y">
												<li class="dropdown-item">
													<a href="<bean:write name="requestVar" property="contextPath"/>/batchDetailAction.do?method=updateBatchAppActivate&scheduleRefId=<bean:write name="schedules" property="scheduleRefId"/>&activate=Y" onclick="return activateConfirmation()" title="Activate Batch App Schedule">Activate Schedule</a>
												</li>
											</logic:notEqual>
										</logic:notEqual>
									</logic:notEqual>
				...
				<div id="batchScheduleDiv" class="card-body">
					...
					<html:hidden property="scheduleModel.listNumber" styleId="listNumber"/>
						...
						<div id="freqRow" class="row col-lg-12">
							...
							<div class="col-lg-2">
								<logic:iterate id="freqCodes" name="batchForm" property="frequencyCodes" indexId="count" type="gov.doc.isu.dwarf.model.CodeModel">
							...
							<div id="weeklyRow" class="form-row col-lg-3">
								<div class="form-group">
									...
									<logic:iterate name="batchForm" property="weekDays" id="day">
							...
							<div id="daysCol" class="form-row col-lg-3 custom-control-inline">
								<logic:iterate name="batchForm" property="daysInMonth" id="dayInMonth" indexId="count">
									<div class="custom-control custom-checkbox">
